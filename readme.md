...
1. **The Four Fingers (Index, Middle, Ring, Pinky)**:
   * Checked using relative Euclidean distances, making the logic immune to hand rotation and effective for both left and right hands.
   * A finger is considered **extended** if the straight-line distance from its tip to the wrist (Landmark `0`) is *greater* than the distance from its lower PIP joint (two nodes down) to the wrist.
   * **Formula**: `Distance(lmList[tip_index], lmList[0]) > Distance(lmList[tip_index - 2], lmList[0])` (e.g., comparing landmarks `8` and `6` against `0`).

2. **The Thumb**:
   * Checked using relative distance across the palm, preventing false readings when the hand flips, points up/down, or switches left to right.
   * A thumb is considered **extended** if its tip (Landmark `4`) is further away from the base of the pinky (Landmark `17`) than its lower IP joint (Landmark `3`) is.
   * **Formula**: `Distance(lmList[4], lmList[17]) > Distance(lmList[3], lmList[17])`.
